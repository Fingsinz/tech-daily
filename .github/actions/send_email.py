...
def send_email(username, password, recipient, subject, body, service='foxmail'):
    """
    ...
    Args:
        ...
        service: Email service type ('gmail', 'foxmail', 'qq', 'outlook', 'qiye_qq')
    """
    print(f"Sending email via {service}...")
    
    if service not in EMAIL_SERVICES:
        raise ValueError(f"Unsupported service: {service}. Supported: {list(EMAIL_SERVICES.keys())}")
    ...
    try:
        ...
        print(f'Authentication failed for {service}. Check username and password.')
        raise
...
<p style="color:#e0e0e0;text-align:center;margin:10px 0 0 0;">{uid()}</p>
    ...
    if models:
        ...
        for i, m in enumerate(models[:10]):
            ...
            html += f'<tr style="background:{bg};"><td style="padding:8px;"><a href="{m["link"]}" style="color:#e65100;">{m["modelId"]}</a></td><td style="padding:8px;text-align:center;">{m.get("downloads", 0):,}</td><td style="padding:8px;text-align:center;">{m.get("likes", 0):,}</td></tr>'
        html += '</table></div>'
    ...
    if articles:
        ...
        for a in articles[:10]:
            html += f'<li style="padding:10px;border-bottom:1px dashed #ce93d8;"><a href="{a["url"]}" style="color:#7b1fa2;text-decoration:none;">{a["title"]}</a> <small style="color:#9c27b0;">by {a.get("user", {}).get("username", "")}</small></li>'
        html += '</ul></div>'
    ...
    if papers:
        ...
        for p in papers[:10]:
            ...
            html += f'<li style="padding:8px 0;color:#5e35b1;"><a href="{p["url"]}" style="color:#512da8;text-decoration:none;">{p["title"]}</a><br/><small style="color:#7e57c2;">{authors}</small></li>'
        html += '</ol></div>'
...
if __name__ == '__main__':
    ...
    service = sys.argv[6] if len(sys.argv) > 6 else 'foxmail'
    ...
    send_email(username, password, recipient, subject, content, service)
